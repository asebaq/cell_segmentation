...
def get_train_transforms(size=480):
    return albumentations.Compose(
        [
            ...
            GridMask(num_grid=3, mode=0, rotate=15, p=0.35),
            GridMask(num_grid=3, mode=2, rotate=15, p=0.35),
            ToTensorV2(p=1.0),
        ]
    )


def get_valid_transforms(size=480):
    return albumentations.Compose(
        [albumentations.Resize(size, size), ToTensorV2(p=1.0)]
    )


def get_test_transforms(size=480):
    return albumentations.Compose(
        [albumentations.Resize(size, size), ToTensorV2(p=1.0)]
    )
